...
```
To my eyes the series have no significant trend. 
### Q2
...
```
To my eye, the differenced series more successfully removed trend then the detrend process. The detrended series shows a general decrease over the first 30 years followed by a sharp rise that ends well above the starting value in 1973. 
...
colnames(renewable.energy.all.series) <- c("Differenced Month", "Differenced Series", "Detrended Series", "Original Month", "Original Series")

renewable.energy.all.series<-renewable.energy.all.series[,-4]
```
...
```{r}
#Use ggplot
ggplot(renewable.energy.all.series, aes(x="Differenced Month"))+
  ...
  labs(title="Total Renewable Energy Production", x="Years", y="Trillion Btu")
...
```{r}
#Compare ACFs
Acf(renewable.energy.all.series[,2],lag.max=40,main=paste("Differenced Series",sep=""))
Acf(renewable.energy.all.series[,3],lag.max=40,main=paste("Detrended Series",sep=""))
Acf(renewable.energy.all.series[,4],lag.max=40,main=paste("Original Series",sep=""))
```
The differencing method appears to be more efficient in eliminating the trend.
...
```{r}
SMKtest.original.series <- SeasonalMannKendall(renewable.energy.all.series[,4])
print(summary(SMKtest.original.series))

print(adf.test(renewable.energy.all.series[,4],alternative = "stationary"))
```
...
#getting an error message I'm not sure how to fix
new.renewable.data.yearly <- data.frame(my_year, renewable.data.yearly)
### Q8
...
```{r}
print(summary(MannKendall(new.renewable.data.yearly)))



print(adf.test(new.renewable.data.yearly, alternative = "stationary"))
```
Got stuck doing the coding I needed to do in order to run Spearman.
sp_rho=cor(renewable.data.yearly, my_year,method="spearman")
print(sp_rho)
